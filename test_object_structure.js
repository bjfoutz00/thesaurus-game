...
// List of words
let syn_list = response.def[0].sseq[0][0][1].syn_list
let rel_list = response.def[0].sseq[0][0][1].rel_list


let all_syns = []
for (list of syn_list) {
    for (wd_obj of list) {
        for (wd in wd_obj) {
            all_syns.push(wd_obj.wd);
        }
    }
}
for (list of rel_list) {
    for (wd_obj of list) {
        for (wd in wd_obj) {
            all_syns.push(wd_obj.wd);
        }
